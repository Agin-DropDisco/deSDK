...
import IDEXswapPair from 'dexswap-core/build/contracts/IDEXswapPair.json'
import IDEXswapFactory from 'dexswap-core/build/contracts/IDEXswapFactory.json'
...
export abstract class Fetcher {
  ...
  private static async fetchTokenLogoUri(token: Token): Promise<string> {
    ...
    if (chainId !== ChainId.MAINNET && chainId !== ChainId.XDAI && chainId !== ChainId.MATIC && chainId !== ChainId.RINKEBY) {
      return ""
    }
  ...
  public static async populateTokenLogoCache(chainId: ChainId): Promise<void> {
    ...
    if (chainId == ChainId.MAINNET) {
      ...
      tokenListURL = 'https://gateway.pinata.cloud/ipfs/QmPgQqbykpBn4EhxkBqNS6DSeqGPku7yvB5fdWvsVohDHJ' // 
    } else if (chainId == ChainId.MATIC) {
      tokenListURL = 'https://gateway.pinata.cloud/ipfs/QmagkvmiCuSgPSmJigdcAGjcERTLt9bYPKsWSE3ADQaxCi' // 
    } else if (chainId == ChainId.RINKEBY) {
      tokenListURL = 'https://gateway.pinata.cloud/ipfs/QmYR2GRPLjxciLbVWMz3jT9wjMv4rsx1y8FoH5egFzPz9Q' // 
    }
